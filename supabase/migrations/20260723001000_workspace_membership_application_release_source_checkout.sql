-- WT-WORKSPACE-TEAM-007-FIX-002 release source editable checkout after successful application.
-- The application release derives the source export from immutable import evidence and
-- reuses the existing checkout release state/audit model with release_source = application_completed.
...
alter table public.workspace_membership_export_runs
  ...
  add constraint workspace_membership_export_runs_release_source_check
    check (release_source is null or release_source in ('holder_undo', 'application_completed'));

create or replace function public.release_workspace_membership_csv_checkout_after_application(
  p_organisation_id uuid,
  p_import_run_id uuid,
  p_application_run_id uuid,
  p_actor_id uuid,
...
declare
  ...
  v_release_reason text := 'Approved Workspace Team changes applied successfully.';
begin
  if p_actor_id is null then
    raise exception 'WT_MEMBERSHIP_APPLICATION_CHECKOUT_RELEASE_ACTOR: Checkout release after application requires an actor.' using errcode = '23514';
  end if;
  ...
  if not found or v_import.status <> 'applied' then
    raise exception 'WT_MEMBERSHIP_APPLICATION_CHECKOUT_RELEASE_IMPORT: Checkout release requires an applied import run.' using errcode = '23514';
  end if;

  select *
    into v_application
  from public.workspace_membership_change_application_runs as ar
  where ar.id = p_application_run_id
    and ar.import_run_id = p_import_run_id
    and ar.organisation_id = p_organisation_id
  for update;

  if not found or v_application.status <> 'applied' then
    raise exception 'WT_MEMBERSHIP_APPLICATION_CHECKOUT_RELEASE_RUN: Checkout release requires an applied application run.' using errcode = '23514';
  end if;

  if v_import.source_export_id is null then
    return null;
  end if;
  ...
  where e.id = v_import.source_export_id
  ...
  if v_source_export.export_mode <> 'editable'
     or v_source_export.status <> 'checked_out'
     or v_source_export.editing_mode <> 'checked_out'
     or v_source_export.released_at is not null
     or v_source_export.superseded_at is not null
     or v_source_export.checkout_expires_at is null
     or v_source_export.checkout_expires_at <= v_released_at
  then
    return null;
  end if;
  ...
  update public.workspace_membership_export_runs as e
    set status = 'released',
        ...
        release_source = 'application_completed',
        release_reason = v_release_reason
  where e.id = v_source_export.id
    and e.organisation_id = p_organisation_id
    and e.export_mode = 'editable'
  ...
  if not found then
    raise exception 'WT_MEMBERSHIP_APPLICATION_CHECKOUT_RELEASE_RACE: The source editable checkout changed before application finalisation could release it.' using errcode = '40001';
  end if;
  ...
  perform public.record_workspace_membership_audit_event(
    ...
    jsonb_build_object(
      ...
      'export_id', v_source_export.id,
      'import_run_id', v_import.id,
      'application_run_id', v_application.id,
      'previous_holder', v_source_export.requested_by,
      'previous_expiry', v_source_export.checkout_expires_at,
      'membership_snapshot_version', v_source_export.membership_snapshot_version
    ),
    jsonb_build_object(
      'organisation_id', p_organisation_id,
...
revoke all on function public.release_workspace_membership_csv_checkout_after_application(uuid, uuid, uuid, uuid, uuid) from public;
revoke all on function public.release_workspace_membership_csv_checkout_after_application(uuid, uuid, uuid, uuid, uuid) from authenticated;
...
begin
  ...
  if v_import.status = 'applied' then
    ...
    if found then
      perform public.release_workspace_membership_csv_checkout_after_application(
        p_organisation_id,
        v_import.id,
...
begin
  for v_row in
    select distinct on (ir.source_export_id)
      ir.organisation_id,
      ir.id as import_run_id,
      ar.id as application_run_id,
      coalesce(ir.applied_by, ar.requested_by, er.requested_by) as actor_id
    from public.workspace_membership_import_runs as ir
    join public.workspace_membership_export_runs as er
      on er.id = ir.source_export_id
     and er.organisation_id = ir.organisation_id
    join public.workspace_membership_change_application_runs as ar
      on ar.import_run_id = ir.id
     and ar.organisation_id = ir.organisation_id
     and ar.status = 'applied'
    where ir.status = 'applied'
      and ir.source_export_id is not null
      and er.export_mode = 'editable'
      and er.status = 'checked_out'
      and er.editing_mode = 'checked_out'
      and er.released_at is null
      and er.superseded_at is null
      and er.checkout_expires_at > now()
      and coalesce(ir.applied_by, ar.requested_by, er.requested_by) is not null
    order by ir.source_export_id, ir.applied_at desc nulls last, ar.completed_at desc nulls last, ar.created_at desc
  loop
    perform public.release_workspace_membership_csv_checkout_after_application(
      v_row.organisation_id,
      v_row.import_run_id,
      v_row.application_run_id,
      v_row.actor_id,
      gen_random_uuid()
    );
  end loop;
...
comment on function public.release_workspace_membership_csv_checkout_after_application(uuid, uuid, uuid, uuid, uuid) is
  'Internal WT-007 application finalisation helper. It derives the source editable checkout from the applied import run, releases only active editable checkouts with release_source application_completed, writes checkout release audit evidence and sends no invitations.';
comment on function public.apply_workspace_membership_change_set(uuid, uuid, uuid) is
  'Applies only the frozen approved Workspace Team CSV change set transactionally and releases the active source editable checkout after successful application evidence is recorded. Release is derived from import evidence, not browser input; invitation delivery remains separate.';
