...
The Risk Register page shows one compact table-led register with stable graphical project-level summary cards for Open risks, Need action and Highest exposure, followed by Active risks, Need action, Draft and Closed views plus a compact Needs Action panel and exposure distribution chart. The default landing view is Active risks, so Draft and Closed records remain accessible without being mixed into live-risk operations. Summary cards, the panel and the chart are derived from the overall authorised project risk dataset and do not change when the table is narrowed by search, filters or pagination. Open risks and Draft risks are lifecycle counts; Draft count appears in the Draft tab rather than a duplicate summary card, with Amber for one to five drafts and Red for more than five drafts. Highest exposure is calculated from assessed active risks only and uses Critical, High, Medium and Low exposure labels; Low remains yellow and is not a Green action state. The exposure distribution chart shows assessed active risks by Critical, High, Medium and Low, includes Closed risks as a separate segment, excludes Draft records, uses assessed active risks plus closed risks for percentages, and calls out unassessed active risks separately. Draft and Closed reference pills are neutral grey reference-only pills and do not visibly show `NOT ACTIVE`; lifecycle remains available in the lifecycle/status column and accessible labels. Draft exposure is provisional estimated exposure and new compatibility-default Drafts display as Unassessed. Closed rows display no current exposure in the register. Need action is driven by Red/Amber action state for active risks, not by exposure alone: a controlled Critical risk with Green action state does not appear solely because of exposure, while a Low exposure risk with Red/Amber action state does appear. The Need action summary card counts active risks; the Needs Action panel lists specific derived work items such as assigning ownership, adding response plans, setting or completing reviews, assessing exposure and updating stale risks. Draft, Closed and terminal records do not generate panel items, and one active risk may generate multiple items. Users can combine case-insensitive reference/title search with Exposure, Action state, Owner and Lifecycle filters. Filtering and sorting apply before pagination. The main table defaults to 25 rows per page, supports 10, 25 and 50 rows per page, normalises invalid page query values, and resets to page 1 when tab/search/filter/sort/page-size controls change. Active risks default to highest exposure first, Need action defaults to action-needed order, Draft defaults to estimated exposure priority then oldest Draft first, and Closed defaults to most recently updated/closed first; explicit user-selected sort options still override those defaults for the selected view. Risk sequence/reference remains the deterministic final tie-breaker. The risk reference pill is the row entry point and carries the derived action-state colour for Active risks only, while exposure remains a separate exposure-only pill and lifecycle/status remains its own column. Permitted users can open/publish Draft risks, close Active risks and reopen Closed risks through server-side `risk.edit` checks from the detail route. Draft saves do not create Project Narrative entries. Opening/publishing, closing and reopening create source-linked Narrative entries, and close/reopen/open notes are captured in risk notes when supplied. Viewers can read lifecycle state and history but cannot trigger lifecycle changes. Summary cards, the Needs Action panel and the exposure chart are risk-position summaries only and do not represent Project Health, which remains Unknown.
...
The project responsibility foundation stores assignments in `project_people`. Each assignment links one project role to either a real active workspace member (`organisation_members.user_id`, aligned with the immutable `profiles.id` person UUID; sign-in is linked separately through `auth_user_id` after invitation Auth repair) or an active `workspace_demo_people` demo persona. Demo assignments remain visibly labelled as demo/persona records. Project roles describe accountability/context only and do not grant project edit rights or change workspace RBAC.
...
WT-RISK-004A refines the same detail page structure without changing the risk model. The risk reference remains the primary page heading but uses the action-state pill treatment at hero-heading scale. The Current risk panel becomes a compact summary strip for action state, lifecycle status, owner and audit metadata rather than a second set of large detail cards.

The main editable detail area is named Current Risk Detail. Detail cards remain focused modal entry points, and their native dialog backdrop uses a dark blurred overlay so the active edit task is visually foregrounded. Comments now sit at the bottom of Current Risk Detail instead of in a separate large block, keeping the page ready for a future Custom Fields section below Comments without implementing custom fields in this slice.
...
## WT-RISK-005 Risk exposure and action state

WT-RISK-005 introduces derived risk action state while keeping lifecycle status, risk exposure and project health separate. WT-RISK-REG-UX-001 makes the display contract explicit. Lifecycle status remains the workflow state. Risk exposure is the assessed seriousness of the risk; for the Watchtower Default MVP model, exposure labels are Critical, High, Medium and Low. Critical uses red styling, High uses orange, Medium uses amber and Low uses yellow. Low exposure must not use green in the default MVP UX. This default is not intended to be the only future model: later configurable risk assessment profiles may define dimensions, scoring values, thresholds, labels, evidence requirements, review cadence and escalation rules. Assurance is derived from missing or weak governance/control data. Risk action state is shown in the header and register reference pills and indicates whether action is needed or recommended now. Action state uses Red, Amber and Green: Red means action needed, Amber means action recommended and Green means no current action due. Green action state does not mean the risk itself is green. Register exposure pills show only the exposure label and colour, not action-state wording.
