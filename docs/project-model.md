...
The Risk detail page no longer shows a second "overall concern" block that repeats the top reference pill. It shows exposure in Current Risk Detail, then shows an action-state rationale block with explanation bullets such as missing owner, missing actioner, overdue review date, missing mitigation or exposure under the default assessment. Project health remains separate and Unknown until a formal health model is implemented.
