...
WT-PROJ-DETAILS-SIGNALS-001 extends Project Details from a single page-level rationale into section-level readiness signals. Project Identity, Description, Context, Dates, Governance/Escalation and Roles/Responsibilities each show their own Red/Amber/Green/Unknown/Neutral state and local rationale, while System Metadata remains informational unless a real integrity concern exists. Green sections are visible but subdued so users can see that expected details are present. The top Project setup attention panel is only a summary of section-owned Red/Amber reasons, preserving the rule that parent attention must be explainable on the destination page. Date attention is project-status aware: a Proposed project missing a target end date is Amber planning incompleteness, while an Active project missing a target end date remains Red delivery-control attention.
