...
WT-RISK-002A introduced the first usable Risk Management surface: a project-scoped Risk Register and risk detail page. WT-RISK-REG-UX-002 turns the Risk Register foundation into a compact table-led register, so source-of-truth project risk records can be scanned in one place with their human-readable risk reference/action-state pill, title, exposure, lifecycle/status, owner, review due date and latest update. WT-RISK-REG-UX-003 adds All risks, Need action, Draft and Closed views plus case-insensitive search, Exposure/Action state/Owner/Lifecycle filters, reset, and compact sort options. WT-RISK-REG-UX-004 adds graphical summary cards above those controls for Open risks, Need action and Highest exposure, while Draft volume stays in the Draft tab count and uses Amber/Red count treatment when draft backlog needs management attention. WT-RISK-REG-UX-005 adds a compact Needs Action panel beside the register on wider screens, showing a limited highest-priority queue of specific active risk work such as assign owner, add mitigation, set review date, review overdue risk, assess exposure and update stale risk. WT-RISK-REG-UX-006 adds URL-backed pagination and compact row density to the main table: search, filters and sort run first, the default page size is 25, supported page sizes are 10, 25 and 50, and page-size/control changes reset the table to page 1. WT-RISK-REG-UX-007 adds a compact server-rendered exposure distribution chart beside the register. It shows active assessed risks by Critical, High, Medium and Low exposure, uses red/orange/amber/yellow respectively, includes Closed risks as a separate segment, calls out unassessed active risks separately, excludes Draft records, and remains stable while the table is searched, filtered, sorted or paginated. WT-RISK-REG-UX-008 adds bounded long-text previews and read-only full-text dialogs to Core Risk detail for risk description, mitigation plan and contingency plan. Summary cards, the panel and the chart use the overall project-scoped risk dataset rather than the current table page. Open risks and Draft risks are lifecycle counts, the Need action card counts active Red/Amber risks, and the Needs Action panel lists derived action items from active risks only, so one risk can produce multiple panel items. Critical/High exposure alone does not create a panel item, while lower-exposure active risks can appear when a specific action is due. Highest exposure uses assessed active Critical/High/Medium/Low exposure. The default sort prioritises exposure, then action state, then review due, then most recently updated, with risk sequence/reference as the stable final tie-breaker. Need action is based on Red/Amber action state rather than exposure alone. WT-RISK-003 adds a nullable single actioner assignment on the risk record while keeping that assignment out of the register table. WT-RISK-004 turns detail assurance cards into focused edit entry points and adds top-level Comments, while WT-RISK-004A and WT-RISK-004B refine the detail page around a compact Current risk strip and Current Risk Detail content area. WT-RISK-005 changes the reference pill into a derived risk action-state indicator, and WT-RISK-REG-UX-001 makes the terminology contract explicit: probability and impact derive exposure as Critical, High, Medium or Low through the Watchtower Default MVP assessment, governance/control quality derives assurance, and action state uses Red, Amber and Green to show whether action is needed or recommended now. Low exposure uses yellow rather than green. The Risk detail page shows exposure as current risk detail and explains action state in a separate rationale block rather than duplicating the same Red/Amber/Green pill. Future Risk Register UX slices will add a risk prompt workflow; targeted partial refresh for chart/register panels and richer text interactions remain post-MVP architectural enhancements rather than SPA behaviour.

WT-RISK-REG-UX-009 renames the operational default tab to Active risks, which excludes Draft and Closed records unless the user explicitly opens their tabs. Draft and Closed reference pills are neutral grey reference-only pills and no longer visibly show `NOT ACTIVE`. Draft exposure is treated as provisional Estimated exposure; compatibility-default Drafts display as Unassessed while non-placeholder Draft estimates are preserved. Closed risks show no current exposure in the register. Active, Need action, Draft and Closed use context-specific default sorting, with Draft ordered by estimated exposure then oldest first and Closed by most recently updated/closed first. This is display normalisation only and requires no migration.

WT-RISK-LIFECYCLE-003 adds the minimum Draft activation gate. Draft risks can be activated only to Open, and only after title, meaningful description, active owner, deliberate probability/impact assessment and a non-overdue review date are present. Actioner, due date, mitigation and contingency remain post-activation assurance gaps rather than activation blockers. Blocked activation creates no risk update or Narrative entry; successful activation creates the existing `Risk opened:` entry.
...
Current Risk Detail keeps overview cards scannable by showing bounded previews for long free-text fields such as risk description, mitigation plan and contingency plan. Full populated text remains available in a read-only accessible dialog from the card area, with focus returned to the trigger on close, while missing text still shows the existing action prompt in the overview. This is only a presentation behaviour; it does not change exposure, action state, lifecycle, project health, edit permissions or stored risk content.
...
WT-DASH-RISK-001 makes the project dashboard Risk tile a light assurance surface. Only the Risk tile icon changes colour, based on the active-only project risk assurance state: any active Red risk is Red, one or more active Amber risks with no Red is Amber, all active Green risks are Green, and no active risks is Neutral. Draft and Closed risks are excluded, and multiple Amber risks do not escalate to Red. Risk exposure remains separate inside Risk Management and does not colour the dashboard icon. Red/Amber Risk tile signals route to the Risk Register Need action view; Green/Neutral signals route to Active risks. Red/Amber count dots, badges, notifications, attention items and health scoring remain out of scope.
