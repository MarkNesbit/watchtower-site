...
The page follows the authenticated project page design standard in `docs/ui-page-design-standard.md`. The hero contains Workspace context, project context, the **Project Narrative** title, and its helper text. The primary **New Entry** action sits in the main content panel header. Owners, admins, and members can open the create modal. Viewers see the same action disabled with helper text explaining that their role does not allow entry creation. A compact filter/status panel remains visible as a future control area and is labelled as coming soon.
