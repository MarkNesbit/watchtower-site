...
There is deliberately no Type or separate Attention column. The internal `entry_number` remains audit/export-readiness data and is not a visible row-number column. Manual and other non-risk entries display `source_ref` when present and otherwise `narrative_ref` as a RAG/attention pill with a visible stored attention label. Risk-linked entries display the current linked risk reference and current action-state colour instead of replaying the stored Narrative `attention_level`. Active linked risks use compact visible `R`, `A` or `G` status labels with the full Red, Amber or Green action state preserved in accessible text and the title. Draft and Closed linked risks show neutral reference-only pills with lifecycle context in accessible text. Missing or inaccessible linked risks show the stored `source_ref` where available, use a neutral unavailable label and do not expose a risk detail link. Clicking an available Ref opens a read-only detail modal without navigating away from the Project Narrative page.

The Ref pill displays both text and a colour treatment, including a quieter neutral state, so attention or action state is not conveyed by colour alone. Details render title and body as escaped Astro template content. Creator display uses profile name/email where the existing profile RLS relationship makes it available and otherwise shows `Workspace member`; UUIDs are not exposed. Creation timestamps currently use a simple explicit UTC display. A shared effective-viewer-timezone DTS helper remains future work.
...
- an existing active risk changes from non-Red to Red using the WT-RISK-005 derived risk action state.
...
Draft saves and routine risk edits do not populate Project Narrative. Description edits, owner changes, actioner changes, review-date or due-date changes, mitigation or contingency changes, comments, Green to Amber movement, Amber to Green movement, Red to Amber/Green movement, and Red staying Red are not Narrative events unless the derived risk action state changes from non-Red to Red on an active risk. Risk comments remain on the risk record and do not create Narrative entries. Close/reopen/open notes can be captured in `project_risk_notes` when supplied, while the source-linked Narrative entry remains concise and neutral for close/reopen.

Risk remains the source of truth. Project Narrative stores only concise event context and source linkage, not a duplicate risk data store or historical replay. Existing Narrative text, details, source metadata, stored attention level and read-state remain historical. The visible risk-linked row pill is deliberately current-state presentation derived from the linked risk at read time. This slice does not introduce attention items, notifications, daily digests, health scoring, AI summaries, issue creation, or risk editing from the Narrative modal.
