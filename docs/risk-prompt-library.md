...
Prompt-created risks are raised Draft records in a basic-capture state, and WT-RISK-LIFECYCLE-002 aligns manual Raise a risk with the same initial Draft outcome. Prompt-created Drafts are visible in the Risk Register and Draft tab immediately, but they still need project-specific detail, assessment and ownership before becoming Active/Open. They are created with `status='draft'`, no owner or actioner, no review or due date, no mitigation or contingency plan, and the same safe required probability/impact storage defaults used for draft compatibility. In the register those compatibility-default Drafts display as Unassessed rather than Medium, while later non-placeholder Draft values may be shown as provisional Estimated exposure for triage. The stored `rag_status='blue'` value is technical compatibility for the legacy required column only; Draft must not be presented as an assessed Blue risk. Prompt-created and manual Drafts do not appear in Needs Action as active work, do not create Project Narrative entries and do not send notifications.
