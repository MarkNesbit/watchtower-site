...
The Risk Register is ordered by management priority. Active risks appear first in the Active Risk Register. Draft risks appear below in a visible Draft risks section when they exist, styled neutrally and separate from active assurance. Closed risks are available after Draft risks in a collapsed Closed risks section so audit/history remains accessible without cluttering active management. Empty Draft and Closed sections are not shown. The register displays concise columns: Ref, Risk, Lifecycle, Status, Review date and Updated. The Risk column shows the title only, not the description. The separate RAG, Owner and Actioner columns are intentionally removed from the primary register table. Active risk reference pills show risk action state rather than raw exposure or the stored legacy `rag_status` value; Draft and Closed reference pills display neutral lifecycle state. Detailed exposure, governance quality and action-state rationale are handled on the risk detail page.

The risk detail page is now an actionable assurance view focused on what needs attention. It displays simple Green, Amber, Red, Neutral and Unknown indicators for summary, lifecycle status, exposure, owner, action responsibility, review cadence, due date, mitigation plan, contingency plan and latest update. Each editable detail card opens a focused edit modal for the relevant source-of-truth fields and posts through the existing scoped edit route. Risk exposure is the assessed seriousness of the risk. For MVP, Watchtower derives exposure through the current default Watchtower assessment using probability and impact: Low/Low is Green, Medium/High, High/Medium and High/High are Red, other valid combinations are Amber, and missing or unknown exposure data is Red. This default exposure model is deliberately named as a current/default assessment so future configurable risk assessment profiles can define dimensions, labels, thresholds, evidence needs, review cadence and escalation rules without replacing every UI concept. Risk action state is the visible Red/Amber/Green indicator for whether action is needed now; it may consider exposure, but it is not the same thing as exposure. Missing owner, required action responsibility, contingency plan, Materialised status, and unresolved Escalated status are Red for active risks; missing review date or due date is Amber for active risks; overdue review date is Red for active risks; missing mitigation is Red for Red exposure, Amber for Amber exposure, and Green for Green exposure in this MVP. Draft and Closed risks keep exposure visible but present assurance fields as Neutral because they are not active assurance records. The detail page shows action-state rationale as explanation bullets rather than duplicating the top Red/Amber/Green pill. These signals are not the final Governance Profile / Assessment Profile engine and do not alter project health or create attention/notification side effects.
...
WT-RISK-005 separates lifecycle status, risk exposure, assurance quality and risk action state. Users update structured facts, Watchtower derives exposure from the current default probability/impact assessment, derives assurance from governance/control quality signals, and derives the action state shown in the risk reference pill from exposure plus assurance rules. The rationale block explains the drivers behind that action state. Owner/actioner inactivity rules are documented as future-ready because `profiles.last_login_at` is available in the schema but the current `record_auth_audit_event('user.logged_in')` flow does not reliably maintain it. Temporary actioner handover/delegation also remains future scope and will need temporary actioner, handover reason, start date, end date, assigned by and original actioner fields.
