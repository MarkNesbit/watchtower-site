...
WT-RISK-REG-UX-002 moves the Risk Register foundation toward the table-led redesign direction. The page now uses one compact all-risk register table rather than stacked Active, Draft and Closed sections. Active risks are ordered first, Draft risks next and Closed risks last, so draft and historical records remain visible without being presented as active assurance items. The register displays concise columns for Ref, Risk, Exposure, Action state, Lifecycle/Status, Owner, Review due, Updated and Actions. Exposure uses the Watchtower Default MVP exposure labels and colours from WT-RISK-REG-UX-001. Action state is separate and uses Red/Amber/Green only for active action-needed state; Draft and Closed rows show neutral non-active action state. Lifecycle/status remains its own column and must not substitute for exposure or action state. This slice is the foundation layout only: tabs, filters, advanced sorting, summary cards, needs-action panels, pagination and exposure distribution charts remain future WT-RISK-REG-UX slices. The same table-led pattern is intended to inform later RAID modules, but those modules remain out of scope.
