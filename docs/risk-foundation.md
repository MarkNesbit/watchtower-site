...
WT-RISK-GUIDE-002 adds the Risk Register `Use risk prompts` modal. It reads the active default library, active areas and active prompts from the seeded database records, renders areas as tabs, and maintains prompt selections in client-side memory using stable `risk_prompt_id` values. Per-area badges and the total count are derived from that temporary state and reset when the modal closes. This slice remains read-only: viewers can review and temporarily select prompts, no project risks are created, Show selected only remains disabled, and Draft risk creation is deferred to WT-RISK-GUIDE-005.
