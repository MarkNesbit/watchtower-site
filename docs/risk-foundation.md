...
**Status:** WT-RISK-LIFECYCLE-001 draft, active, closed and reopened risk handling; WT-RISK-LIFECYCLE-001A assessment complete; WT-RISK-LIFECYCLE-001B shared lifecycle/action-state contract hardened; WT-RISK-LIFECYCLE-002 manual and prompt Draft capture aligned
...
The current database keeps `probability` and `impact` non-null, so untouched manual and prompt-created Drafts continue to store the existing Medium/Medium compatibility values and display as `Unassessed` through `getRiskRegisterExposureDisplay`. Risk Detail consumes that same Draft display interpretation for its exposure card and says that no estimated exposure has been recorded, rather than presenting Medium probability / Medium impact as deliberate assessment. If a user deliberately supplies non-placeholder probability/impact values at creation or later edit, the Draft can show a provisional estimated exposure. Known limitation: under the current schema Watchtower cannot reliably distinguish an untouched compatibility Medium/Medium Draft from a deliberately assessed Medium/Medium Draft. That ambiguity is documented for the future activation/readiness design rather than resolved with a migration in this slice.
...
Owner, Admin and Member roles may create and edit risks when the `riskManagement` feature flag permits access. Manual create now captures a lightweight Draft risk: title is required, while description, probability, impact, owner, actioner, review date, due date, mitigation plan and contingency plan are optional. The server always assigns `status='draft'` during initial creation, ignores submitted create-status values, and generates the next project-scoped reference in `Risk-{PROJECT_REF}-{NNN}` format. Saving a new Draft risk creates only the Risk record and redirects to the Draft detail page with `Draft risk created.` confirmation; it does not create a Project Narrative entry. Edit allows the same editable fields, including lifecycle status for the current MVP, to be updated while preserving immutable scope and creation fields. Lifecycle status edits that move Draft -> Active, Active -> Closed or Closed -> Active create the same open/close/reopen Narrative entries as the detail-page lifecycle controls. The database audit trigger binds `created_by` and `updated_by` to the authenticated user. The `project_risks.rag_status` column remains as legacy/transitional compatibility storage, but user-facing flows no longer treat it as a manually declared source of truth.
