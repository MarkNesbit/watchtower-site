...
**Status:** WT-RISK-LIFECYCLE-001 draft, active, closed and reopened risk handling; WT-RISK-LIFECYCLE-001A assessment complete; WT-RISK-LIFECYCLE-001B shared lifecycle/action-state contract hardened; WT-RISK-LIFECYCLE-002 manual and prompt Draft capture aligned; WT-RISK-LIFECYCLE-003 minimum Draft activation gate implemented; WT-RISK-LIFECYCLE-004 Draft Register readiness implemented; WT-RISK-LIFECYCLE-005 Dashboard and Project Signal alignment implemented; WT-RISK-LIFECYCLE-006 Narrative current-state risk pills implemented
...
WT-RISK-LIFECYCLE-001B-FIX-001 corrects active-risk review-date cadence inside that shared contract. Active risks with no review date are Amber. Overdue review dates and review dates due today are Red. Review dates due tomorrow or within the next three calendar days are Amber. Review dates later than that are Green. The three-day window is an MVP application constant and may later become configurable through Governance Profiles. This defect fix has no database migration or production data impact.

WT-RISK-LIFECYCLE-002 aligns manual and prompt initial risk capture under the same parent Epic. Manual Raise a risk and guided prompt creation now both create `project_risks` records in `draft` status, with normal project-scoped references and audit metadata. Manual Draft capture requires only a title; description, probability, impact, owner, actioner, review date, due date, mitigation plan and contingency plan are optional and retained when supplied. The create form no longer exposes lifecycle/status selection and clearly states that the risk will be created as a Draft for review and assessment. Server-side creation enforces `status='draft'` and ignores any submitted create-status payload. WT-RISK-LIFECYCLE-002-FIX-001 keeps Risk Register and Risk Detail exposure presentation consistent for untouched Drafts by reusing the shared Draft exposure display contract: compatibility Medium/Medium Drafts display as Unassessed on both surfaces. This slice does not introduce the activation gate, transition hardening on edit, organisation-specific rules, new migrations or production data changes.

WT-RISK-LIFECYCLE-003 implements the minimum Draft activation gate. A Draft risk can become Active only by activating to `open`, and only when it has a title, a project-specific description of at least 30 characters, an active owner, a deliberate probability/impact assessment and a review date that is not overdue. Actioner, due date, mitigation plan and contingency plan remain post-activation assurance areas and do not block activation. The shared readiness helper is used by the detail-page activation panel, `transitionProjectRiskLifecycle` and `updateProjectRisk`, so crafted edit submissions cannot bypass the gate or move Drafts directly to Monitoring, Mitigating, Escalated, Materialised or Closed. Blocked activation performs no risk update and creates no Narrative entry. Successful activation creates the existing `Risk opened:` Narrative entry and does not create a duplicate Red entry. The slice adds nullable `assessment_completed_at` and `assessment_completed_by` fields so compatibility Medium/Medium Draft defaults are not treated as a deliberate assessment; no production data is backfilled.

WT-RISK-LIFECYCLE-004 adds Draft readiness and age presentation to the Risk Register Draft tab. Draft rows show estimated exposure, activation readiness, missing activation requirements through an accessible expandable row detail, and calendar-day Draft age. Drafts aged 14 calendar days or more show a neutral `Review Draft` backlog hygiene challenge. These signals reuse `getRiskActivationReadiness` and do not change Draft action-state neutrality, Needs Action, dashboard Risk signals, project attention, Narrative generation, permissions, workflows, migrations or production data.

WT-RISK-LIFECYCLE-005 aligns the dashboard Risk tile, project risk signal helpers, Risk Register Needs Action routing and Risk Detail return navigation. Project-level risk signal now uses one shared active-only contract: any active Red risk makes the signal Red, one or more active Amber risks with no Red makes it Amber, one or more all-Green active risks makes it Green, and no active risks makes it Neutral. Multiple Amber risks do not escalate to Red. Draft, Closed and legacy terminal statuses are excluded. The dashboard Risk tile links to the Risk Register Need action view for Red/Amber risk signals and the Active risks view for Green/Neutral signals. Risk Register detail links pass only a validated `returnTab` identifier, and Risk Detail/Edit use that tab to return to the originating Register view without accepting arbitrary return URLs. No lifecycle transitions, exposure rules, Project Health policy, Narrative behaviour, migrations or production data changed.

WT-RISK-LIFECYCLE-006 makes risk-linked Project Narrative row pills present the current linked risk action state while preserving historical Narrative title, detail, source metadata, attention level and read-state. Active linked risks use the same shared risk reference pill contract as the Risk Register and detail hero: visible compact labels are `R`, `A` and `G`, while the full Red, Amber or Green action state remains in the accessible label and title. Draft and Closed linked risks show neutral reference-only pills with lifecycle context in accessible text. Missing or inaccessible linked risks show the stored `source_ref` where available, use a neutral unavailable label and do not create an unsafe risk detail link. This slice has no migration, no production data change and no new Narrative generation trigger.
...
- explicit Draft assessment completion through nullable `assessment_completed_at` and `assessment_completed_by`;
...
WT-RISK-LIFECYCLE-001B records the authoritative lifecycle grouping used by active action-state consumers: Draft is `draft`; Active is `open`, `monitoring`, `mitigating`, `escalated` and `materialised`; Closed is `closed`, with `accepted`, `resolved`, `passed`, `retired`, `cancelled` and `rejected` retained as safe terminal compatibility values. Active risk action state rolls up from assurance areas as Red when any area is Red, Amber when one or more areas are Amber and none are Red, Green when all applicable areas are Green, and Neutral for Draft or Closed risks. Multiple Amber areas deliberately remain Amber for the MVP. Risk Detail, Risk Register Needs Action, the dashboard Risk tile, project-list attention aggregation and risk-linked Narrative row pills consume this shared lifecycle/action-state contract rather than treating exposure, stored `rag_status`, lifecycle status, stored Narrative attention or project health as substitutes.

The active review-date cadence rule is part of that shared contract: no review date is Amber, an overdue review date or a review date due today is Red, a review date due tomorrow or within the next three calendar days is Amber, and a later review date is Green. Due-date policy is separate and was not redesigned by WT-RISK-LIFECYCLE-001B-FIX-001.

WT-RISK-LIFECYCLE-002 keeps Draft creation neutral for action-state purposes. New manual Drafts appear in the Draft tab, stay out of Active risks, Need action, dashboard Risk tile signals and project-list attention aggregation, and do not create a Project Narrative entry. The later Draft -> Open transition remains the point at which the existing `Risk opened:` Narrative entry is created. Prompt-created Drafts retain `source_risk_prompt_id`; manually created Drafts leave that field null.

The current database keeps `probability` and `impact` non-null, so untouched manual and prompt-created Drafts continue to store the existing Medium/Medium compatibility values and display as `Unassessed` through `getRiskRegisterExposureDisplay`. Risk Detail consumes that same Draft display interpretation for its exposure card and says that no estimated exposure has been recorded, rather than presenting Medium probability / Medium impact as deliberate assessment. WT-RISK-LIFECYCLE-003 adds `assessment_completed_at` and `assessment_completed_by` as the explicit marker for deliberate Draft probability/impact assessment, including deliberate Medium/Medium. Existing records are not backfilled. Drafts without that marker and with compatibility Medium/Medium remain Unassessed; Drafts with a deliberate assessment show provisional Estimated exposure.

Draft activation is now guarded by `getRiskActivationReadiness` and server-side enforcement. The minimum activation gate requires title, meaningful description, active owner, completed assessment marker with valid probability and impact, and a review date that is not overdue. A review date due today passes activation readiness but will immediately show Red action state once active, matching the active review-date contract. Missing actioner, missing due date, missing mitigation and missing contingency are intentionally non-blocking activation gaps; once activated, they may produce Amber or Red action state through the existing assurance/action-state rules. Draft edit keeps the risk in Draft; activation is handled explicitly from the detail page and always lands in `open`. The Risk Register Draft tab uses the same readiness helper to show `Ready to activate` or the count/list of missing activation requirements, so the Register cannot report a different readiness state from the detail page or server-side gate.

## Dashboard and RAID future scope

The project dashboard Risk tile routes to the project Risk Register when Risk Management is available. Red and Amber risk signals open the Need action view; Green and Neutral signals open the Active risks view. WT-DASH-RISK-001, WT-DASH-TILE-SIGNALS-001 and WT-RISK-LIFECYCLE-005 make the tile a compact attention/assurance signal without turning it into a count, badge or notification surface. The tile uses active risk management gaps rather than raw exposure: a high-exposure risk does not automatically make the tile Red if owner, action responsibility, mitigation/response, contingency and review cadence are current. Risk exposure remains visible in the Risk Register and Risk Detail views.
...
Owner, Admin and Member roles may create and edit risks when the `riskManagement` feature flag permits access. Manual create now captures a lightweight Draft risk: title is required, while description, probability, impact, owner, actioner, review date, due date, mitigation plan and contingency plan are optional. The server always assigns `status='draft'` during initial creation, ignores submitted create-status values, and generates the next project-scoped reference in `Risk-{PROJECT_REF}-{NNN}` format. Saving a new Draft risk creates only the Risk record and redirects to the Draft detail page with `Draft risk created.` confirmation; it does not create a Project Narrative entry. Draft edit preserves Draft status and does not expose direct lifecycle/status selection; the detail page shows activation readiness and an explicit Activate risk action. Active and Closed lifecycle changes still use the scoped server-side edit permission and create the same close/reopen Narrative entries as before. The database audit trigger binds `created_by` and `updated_by` to the authenticated user. The `project_risks.rag_status` column remains as legacy/transitional compatibility storage, but user-facing flows no longer treat it as a manually declared source of truth.

WT-RISK-GUIDE-005 adds guided Draft risk creation from the Risk Register prompt modal; WT-RISK-LIFECYCLE-002 aligns manual creation with that same Draft capture outcome. The route `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/risks/prompt-drafts` accepts selected stable `risk_prompt_id` values, derives workspace and project from the route, enforces the same `risk.create` permission as manual creation, reloads active prompt data from the database, and creates Draft risks using the existing project-scoped reference generator. Prompt titles map to risk titles and prompt guidance maps to risk description. Owner, actioner, dates and planning fields remain null for prompt-created Drafts, and prompt/manual Draft records stay out of active action-state and Needs Action calculations until a user later opens/publishes them through the normal Draft lifecycle. The stored `rag_status='blue'` value for a Draft is technical compatibility with the legacy required column only and must not be displayed or interpreted as an assessed Blue risk.
