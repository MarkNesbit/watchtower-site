...
WT-RISK-LIFECYCLE-002 aligns manual and prompt initial risk capture under the same parent Epic. Manual Raise a risk and guided prompt creation now both create `project_risks` records in `draft` status, with normal project-scoped references and audit metadata. Manual Draft capture requires only a title; description, probability, impact, owner, actioner, review date, due date, mitigation plan and contingency plan are optional and retained when supplied. The create form no longer exposes lifecycle/status selection and clearly states that the risk will be created as a Draft for review and assessment. Server-side creation enforces `status='draft'` and ignores any submitted create-status payload. This slice does not introduce the activation gate, transition hardening on edit, organisation-specific rules, new migrations or production data changes.
...
The current database keeps `probability` and `impact` non-null, so untouched manual and prompt-created Drafts continue to store the existing Medium/Medium compatibility values and display as `Unassessed` through `getRiskRegisterExposureDisplay`. The UI no longer presents those compatibility defaults as a completed assessment during create. If a user deliberately supplies non-placeholder probability/impact values at creation or later edit, the Draft can show a provisional estimated exposure. Known limitation: under the current schema Watchtower cannot reliably distinguish an untouched compatibility Medium/Medium Draft from a deliberately assessed Medium/Medium Draft. That ambiguity is documented for the future activation/readiness design rather than resolved with a migration in this slice.
