...
WT-WORKSPACE-TEAM-008A-FIX-005 provisions invitation Auth identities through the server-side Supabase Auth Admin API before delivery or password setup links are attempted. It reports identity-less historical invitation users with `workspace_invitation_identityless_auth_user_report(organisation_id)`, which returns UUIDs, statuses and email domain only. If the existing placeholder Auth row cannot be repaired in place, Watchtower asks Supabase Auth Admin to quarantine the placeholder email, create a new valid Auth user for the deterministic invitation alias, and then transactionally remap only explicit `auth_user_id` links on the profile, membership and current invitation. `profiles.id` and `organisation_members.user_id` remain the original Watchtower profile/person UUIDs; `profiles.auth_user_id`, `organisation_members.auth_user_id` and `workspace_membership_invitations.auth_user_id` hold the sign-in Auth UUID. Repair is idempotent, service-role only, audited in `workspace_invitation_auth_identity_repairs`, and never writes to `auth.identities` directly.
