...
Internal shared-contact testing is supported only through `workspace_invitation_delivery_policies` with the `internal_gmail_alias` strategy and the existing immutable internal workspace predicate `public.is_internal_role_simulation_workspace`. That strategy preserves the shared `contact_email`, generates a deterministic unique auth alias from the configured base address and login name/profile id, and may force delivery to the shared inbox. It is not inferred from workspace name, slug, email domain or duplicate email occurrence.

The server route `/app/workspaces/{workspaceSlug}/team/invitations/send` supports bulk send, individual send, retry, resend and cancellation for real active Owners/Admins. It creates both HTML and plain-text invitation content. Production email delivery still requires a configured server-side provider; without that provider WT-008 records `provider_not_configured` as `delivery_failed` rather than claiming success. A bounded `WATCHTOWER_INVITATION_DELIVERY_MODE=test_record_only` mode can mark delivery as recorded for internal validation without exposing tokens in logs or browser output.
