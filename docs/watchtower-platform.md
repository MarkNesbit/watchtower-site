...
WT-NARRATIVE-003 adds manual Project Narrative entry creation for owners, admins, and members. Viewers retain read-only access and see the create action disabled with helper text. Manual entries can include structured links with required labels and safe `http://` or `https://` URLs. WT-RISK-NARRATIVE-001 adds deliberately limited risk-generated entries only when a risk is raised or when an existing risk changes from non-Red to Red. Clicking a Narrative Ref opens a read-only detail modal on the same page; for risk-generated entries, the risk reference pill is the accessible trigger for the same modal. The modal shows current source-risk detail with separate exposure, assurance and overall concern signals, and includes an Open full risk in new tab action without enabling risk editing from Narrative.
...
WT-DASH-TILE-SIGNALS-001 makes dashboard capability tiles a shared signal surface while keeping icon-and-title-only tile content. Tile status is separate from overall Project Health. The Project Details tile uses existing setup fields and project-date readiness: missing critical setup or overdue non-start dates are Red, important missing fields or approaching date warnings are Amber, and known complete/current data is Green. A past Start date alone is not Red. The Project Narrative tile is user-specific and based on unseen entries: Green means no unseen entries, Amber means 1-3 unseen entries, Red means 4 or more unseen entries, and Unknown means read-state cannot be calculated safely. A missing read-state treats all existing entries as unseen so a first-time viewer does not see a false Green. Opening the Project Narrative page updates that user's read-state, while dashboard rendering never marks entries as read. The Risks tile uses risk attention/assurance triggers, not raw risk exposure: a high-exposure Red risk does not automatically make the tile Red, and a well-managed high-exposure risk can leave the tile Green when ownership, action responsibility, mitigation/response, contingency and review cadence are current. Risk exposure remains visible in the Risk Register and Risk Detail views.

WT-PROJ-LIST-ATTN-001 makes the Projects page a scan-first assurance entry point. The project name is the primary dashboard link and the former Action column is removed. The project reference pill shows project attention state using the shared RAG pill system: Red means at least one unresolved active Red signal is known, Amber means at least one unresolved active Amber signal is known and no Red signal is known, Green means no active Red or Amber signal is currently known, and Unknown means attention could not be calculated. The first MVP-light calculation is risk-led and reuses existing active risk concern logic; Project Details/date signals, Narrative unseen-entry state, Issues, Dependencies, Assumptions, Actions and Decisions remain future attention sources.
