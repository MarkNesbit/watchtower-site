...
Watchtower uses Red/Amber/Green as an attention and assurance signal, not only as project health. Project Health remains a distinct health assessment; RAG visuals may also describe attention, completeness, review readiness, assurance state or item status. Standard RAG pills must include visible text and standard RAG cards/panels use a subdued state surface, clear left accent border and explicit pill. Dashboard capability tiles may carry a compact RAG/attention state through their tile and icon treatment, but should remain simple and avoid permanent descriptive helper text, count badges, dots or notification-style decoration.
