...
Project-scoped RAID-style registers should prefer compact table-led layouts once the module moves beyond a small proof-of-concept list. The Risk Register foundation uses this pattern first: one register table separates Ref, Risk, Exposure, Action state, Lifecycle/Status, Owner, Review due, Updated and Actions. Its control layer uses compact tabs, search, filters, active filter chips, reset and sort controls above the table. Need action views must be based on action state, not raw seriousness/exposure. Future Issues, Dependencies, Assumptions, Decisions and Actions modules should follow the same separation of record seriousness, action state and lifecycle/status where those concepts exist, without implementing those modules through the Risk Register slice.
