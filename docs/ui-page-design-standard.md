...
Dashboard capability tiles stay understandable from visible title text and accessible labels rather than rollover-only helper text. The Risk tile may use the shared RAG tile treatment to reflect the active-only project risk assurance signal: any active Red risk is Red, one or more active Amber risks with no Red is Amber, all active Green risks are Green, and no active risks is Neutral. Draft and Closed risks are excluded, multiple Amber risks do not escalate to Red, exposure does not drive the state, capability availability remains separate from action state, and tiles must not gain count badges, dots, notification markers or health-scoring decorations. Risk tile navigation should route Red/Amber signals to the Risk Register Need action view and Green/Neutral signals to Active risks.
