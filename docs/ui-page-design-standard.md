...
The Risk Register default tab is Active risks. Draft and Closed rows must be reachable through their own tabs without being mixed into live-risk operations. Draft and Closed reference pills use neutral grey reference-only styling with lifecycle context in accessible labels rather than visible `NOT ACTIVE` text. Draft exposure may be displayed as provisional Estimated exposure, compatibility-default Drafts display as Unassessed, and Closed rows show no current exposure value in the register.
