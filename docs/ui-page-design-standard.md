...
Project-scoped RAID-style registers should prefer compact table-led layouts once the module moves beyond a small proof-of-concept list. The Risk Register foundation uses this pattern first: compact project-level summary cards sit above one register table that separates Ref, Risk, Exposure, Action state, Lifecycle/Status, Owner, Review due, Updated and Actions. Its control layer uses compact tabs, search, filters, active filter chips, reset and sort controls above the table. Need action views and summary cards must be based on action state, not raw seriousness/exposure. Highest exposure summaries must use exposure labels and colours, with Low styled yellow rather than green, and must not imply project health. A side-panel action queue may sit beside the table on wider screens when it shows specific derived work rather than duplicating the register: for Risk Register this is the Needs Action panel, which lists a small number of active-risk actions, remains stable while table filters change, links to risk detail, and excludes Draft/Closed/terminal records. Future Issues, Dependencies, Assumptions, Decisions and Actions modules should follow the same separation of record seriousness, action state and lifecycle/status where those concepts exist, without implementing those modules through the Risk Register slice.
