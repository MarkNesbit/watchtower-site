...
WT-DASH-TILE-SIGNALS-001 makes Project Dashboard capability tiles a shared signal surface. Project Details reflects setup completeness and project-date readiness, Project Narrative remains Unknown until user-specific read-state exists, and Risks uses attention/assurance triggers rather than raw exposure. A high-exposure Red risk can leave the Risks tile Green when ownership, action responsibility, response plans and review cadence are current.
