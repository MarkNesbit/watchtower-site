...
WT-PROJ-DETAILS-SIGNALS-001 makes Project Details section signals explicit. The Project Details page now derives Project Identity, Description, Context, Dates, Governance/Escalation, Roles/Responsibilities and System Metadata states from the shared Project Details signal helper, shows calm Green completion states where expected details are present, and uses the same section-owned Red/Amber reasons in the top Project setup attention summary. Date attention is project-status aware: a Proposed project missing a target end date is Amber planning incompleteness, while an Active project missing a target end date remains Red delivery-control attention. Editable attention items continue to use the existing permission-safe modal save paths.
