...
Project Actions now has a project register at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/actions` and detail pages at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/actions/{actionId}`. WT-ACTION-002 adds the table-led Actions Register with Outstanding, Awaiting review, Complete, Cancelled and All tabs, query-backed search/filter/sort/pagination, project-level Open Actions, Need Action and Highest urgency summary cards, a Needs Action panel, an Action distribution panel, direct Project-sourced Action creation for Owner/Admin/Member roles, Viewer read-only behavior, and read-only immutable history on detail pages. Acceptance-owner management controls use the WT-ACTION-001B RPCs for brief, due date, assignment, reissue, complete, return-to-Actioner, cancel and Owner/Admin takeover. Submit, return-to-raiser and reject Actioner response forms remain deferred to WT-ACTION-003.
