...
Workspace Team at `/app/workspaces/{workspaceSlug}/team` is Owner/Admin-only. Active members open a read-only identity/member-details modal with permission-controlled role management, deactivation, advisory member edit sessions, optimistic snapshot concurrency and audit events. Deactivated members open the same modal for explicit role-decision reactivation where authority permits. Invited members remain non-interactive in this individual-member slice; invitation lifecycle and CSV administration remain separate journeys. Deactivated users may still authenticate, but active workspace membership is revalidated on authenticated routes; users with no active workspaces see a controlled no-access page and are never given fallback personal workspaces merely because access ended.

Risk Management now has a feature-gated register at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/risks`, single-risk actionable assurance detail pages at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/risks/{riskId}`, plus create and edit routes for owner, admin and member roles. WT-RISK-REG-UX-002 moves the register foundation from stacked Active/Draft/Closed sections to a compact all-risk table with action-state reference pills plus separate exposure, lifecycle/status, owner, review due date and updated time columns. WT-RISK-REG-UX-003 adds compact All risks, Need action, Draft and Closed views, case-insensitive search by risk reference/title, Exposure/Action state/Owner/Lifecycle filters, reset, and default sorting by exposure, action state, review due date and latest update. WT-RISK-REG-UX-004 adds stable graphical project-level summary cards for Open risks, Need action and Highest exposure above the table controls; Draft volume is shown in the Draft tab count, with Amber/Red treatment when draft risk counts need management attention. WT-RISK-REG-UX-005 adds a compact derived Needs Action panel that lists the highest-priority live work on active risks, such as assigning owners/actioners, adding response plans, setting or completing reviews, assessing exposure and updating stale risks. WT-RISK-REG-UX-006 paginates the filtered/sorted register table with a default 25 rows per page and 10/25/50 page-size choices, while keeping rows compact and preserving URL state. WT-RISK-REG-UX-007 adds a compact server-rendered exposure distribution chart; Draft records are excluded, closed risks are shown as their own segment, percentages use assessed active risks plus closed risks, unassessed active risks are called out separately, and the chart remains stable while the table is filtered, sorted or paginated. WT-RISK-REG-UX-008 keeps Core Risk detail cards scannable by clamping long risk description, mitigation plan and contingency plan previews and opening the full stored text in read-only accessible dialogs. Filtering and sorting run before pagination; changing tabs, search, filters, sort or page size returns the table to page 1. The Need action summary card counts active Red/Amber risks, while the panel lists specific actions and one risk can generate more than one item. Need action is driven by Red/Amber action state, not raw exposure, and Highest exposure uses assessed active Critical/High/Medium/Low exposure, with Low styled yellow rather than green. Only Active risks drive active assurance and attention; Draft and Closed risks stay visible/auditable but neutral for action state, and reopened risks return to Active handling. WT-RISK-005 and WT-RISK-REG-UX-001 keep lifecycle status, exposure, assurance and action state separate: users edit structured facts, probability and impact derive exposure as Critical/High/Medium/Low, weak governance/control data derives assurance, and active risk reference pills use the derived Red/Amber/Green action state rather than a manually selected RAG. Low exposure uses yellow rather than green in the default MVP UX; Green means no current action due. The stored `rag_status` value remains a legacy/transitional compatibility field until there is a migration strategy. Risk references remain system-generated and read-only; routine risk edits, risk comments, risk delete, replies/threading, persistent attention items, notifications, exposure heatmaps/trends, configurable Governance Profile / Assessment Profile scoring, AI behaviour and health scoring remain deferred. Risk remains the source of truth; Project Narrative is not a risk audit log.

Project Actions now has a project register at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/actions` and detail pages at `/app/workspaces/{workspaceSlug}/projects/{projectSlug}/actions/{actionId}`. WT-ACTIONS-UX-002A simplifies the register around My actions / All project actions, Outstanding, Awaiting review, Complete and Cancelled tabs, Search actions, Actioner and Raiser filters, fixed workflow-specific sorting and Load 20 more row loading. Direct Project-sourced Action creation remains available for Owner/Admin/Member roles, Viewer read-only behavior is preserved, and detail pages continue to show read-only immutable history plus the existing acceptance-owner management controls for brief, due date, assignment, reissue, complete, return-to-Actioner, cancel and Owner/Admin takeover. Submit, return-to-raiser and reject Actioner response forms remain deferred to WT-ACTION-003.
