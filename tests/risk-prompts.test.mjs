...
test('Risk Register prompt modal loads database prompts with tabbed temporary selection state', async () => {
	...
	assert.doesNotMatch(route, /Watchtower will create \$\{createableCount\} new risks in Draft for this project/);
	assert.doesNotMatch(route, /selected prompts already exist and will not be created again/);
	assert.match(route, /risk-prompt-confirmation__metric--selected/);
	assert.match(route, /risk-prompt-confirmation__metric--create/);
	assert.match(route, /risk-prompt-confirmation__metric--duplicate/);
	assert.match(route, /prompts selected/);
	assert.match(route, /Draft risks will be created/);
	assert.match(route, /risks already exist in this project/);
	assert.match(route, /Already in this project/);
	assert.match(route, /New Draft risks/);
	assert.match(route, /risk-prompt-confirmation__section--create/);
	assert.match(route, /risk-prompt-confirmation__section--duplicate/);
	assert.match(route, /risk-prompt-confirmation__section-icon" aria-hidden="true"/);
	assert.match(route, /data-risk-prompt-duplicate-list/);
	assert.match(route, /data-risk-prompt-new-list/);
	assert.match(route, /link\.textContent = ref/);
	assert.match(route, /link\.target = '_blank'/);
	assert.match(route, /link\.rel = 'noopener noreferrer'/);
	assert.match(route, /risk-prompt-confirmation__risk-ref/);
	assert.match(route, /risk-prompt-confirmation__risk-title/);
	assert.doesNotMatch(route, /link\.textContent = 'Open'/);
	...
	assert.doesNotMatch(route, /View existing risks/);
	assert.doesNotMatch(route, /data-risk-prompt-view-existing/);
