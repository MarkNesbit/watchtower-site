...
test('Project Narrative page provides manual creation and read-only detail modal behaviour', async () => {
	...
	assert.match(page, /getNarrativeReferencePillPresentation\(entry, linkedRisk, narrativeNow\)/);
	assert.match(page, /data-risk-detail-trigger=\{referencePill\.riskDetailAvailable \? '' : undefined\}/);
	assert.match(page, /View current risk detail for \$\{referencePill\.label\}/);
	...
	assert.match(page, /sourceRisksById\.get\(entry\.source_record_id\)/);
	...
	assert.doesNotMatch(page, /buildProjectRiskPath\(workspaceSlug \?\? '', project\?\.slug \?\? projectSlug \?\? '', entry\.source_record_id\)/);
	assert.match(page, /deriveRiskAssuranceTone\(risk, narrativeNow\)/);
	assert.match(page, /deriveRiskExposureTone\(risk\.probability, risk\.impact\)/);
	assert.match(page, /riskExposureToneLabel\(exposure\)/);
	assert.match(page, /deriveRiskReferenceTone\(risk, narrativeNow\)/);
	assert.match(page, /riskReferenceStatusLabel\(risk, narrativeNow\)/);
	assert.match(page, /entry\.sourceRisk\.available === false/);
	assert.match(page, /openRisk\.hidden = entry\.sourceRisk\.available === false \|\| !entry\.sourceRisk\.openHref/);
	assert.match(page, /openRisk\.removeAttribute\('href'\)/);
	...
	assert.match(page, /import \{ getNarrativeReferencePillPresentation \}/);
	assert.match(page, /getNarrativeDisplayRef\(entry\)/);
	assert.doesNotMatch(page, /getAttentionPillTone/);
	assert.match(page, /<RagReferencePill[\s\S]*tone=\{referencePill\.tone\}[\s\S]*label=\{referencePill\.label\}[\s\S]*statusLabel=\{referencePill\.statusLabel\}[\s\S]*compactActionState=\{referencePill\.compactActionState\}/);
	assert.match(page, /aria-label=\{referencePill\.riskDetailAvailable[\s\S]*\? `View current risk detail for \$\{referencePill\.label\}`[\s\S]*: referencePill\.ariaLabel/);
	assert.match(ragReferencePill, /statusLabel/);
	assert.match(ragReferencePill, /compactActionState/);
	assert.match(ragReferencePill, /visibleStatusLabel/);
