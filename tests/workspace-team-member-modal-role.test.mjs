...
test('Workspace Team member session and role routes use workspace-scoped secure RPCs', async () => {
	...
	assert.doesNotMatch(page, /Astro\.locals\.runtime|runtime\?\.env|runtimeEnv/);
	assert.match(page, /createSupabaseServerClient\(accessToken\)/);
	assert.match(sessionRoute, /cookies, params, request/);
	assert.match(sessionRoute, /createSupabaseServerClient\(accessToken\)/);
	assert.match(sessionRoute, /getWorkspaceBySlug\(serverSupabase, workspaceSlug, accessToken\)/);
	assert.match(roleRoute, /cookies, params, request/);
	assert.match(roleRoute, /createSupabaseServerClient\(accessToken\)/);
	assert.doesNotMatch(sessionRoute, /Astro\.locals\.runtime|runtime\?\.env|runtimeEnv/);
	assert.doesNotMatch(roleRoute, /Astro\.locals\.runtime|runtime\?\.env|runtimeEnv/);
